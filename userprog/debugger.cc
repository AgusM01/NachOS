...
CommandDump(char **args, void *extra)
{
    ...
    int memorySize = machine->GetNumPhysicalPages()*PAGE_SIZE;
    int rv = fwrite(machine->mainMemory, 1, memorySize, f);
    if (rv != memorySize) {
...
CommandPrint(char **args, void *extra)
{
    ...
    while ((arg = DCM::FetchArg(args)) != nullptr) {
        ...
        if (strcmp(end, "@v") == 0 || *end == '\0') {
            ...
            unsigned memorySize = machine->GetNumPhysicalPages()*PAGE_SIZE;
            if (address >= memorySize) {
                fprintf(stderr, "ERROR: address %u is too big.\n", address);
                return DCM::RUN_RESULT_STAY;
            }

            PrintChar(machine->mainMemory[address]);
